...
class Joystick {
	constructor(x, y, radius, handleRadius) {
		...
		this.posZB = new Vector2(x, y); // position zero based
	...
	listener() {
		...
		addEventListener('touchstart', e => {
			...
			// Check if touch is inside the joystick
	...
	reposition() {
		...
		if (!this.ondrag) {
			...
			if( this.posZB.x < 0.001 )
				this.posZB.x = 0.0;

			this.posZB.y = this.origin.y - this.pos.y;
			if( this.posZB.y < 0.001 )
				this.posZB.y = 0.0;

			// debug output
			// myStr = "X: " + this.posZB.x + ", Y: " + this.posZB.y;
			// console.log(myStr);
		} else {
